...
export function configuration(env = process.env) {
  ...
  return { values, checks, ready: Object.values(checks).every(Boolean) };
}

export function isPublicCallback(value) {
  try {
    const url = new URL(value);
    return url.protocol === 'https:' && url.pathname.endsWith('/api/oauth-callback') && !['localhost', '127.0.0.1'].includes(url.hostname);
  } catch {
    return false;
  }
}
