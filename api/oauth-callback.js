...
function htmlPage({ title, message, profile, content, stateVerified, error = false }) {
  ...
  const contentBlock = content ? `<section class="card"><p class="label">最近创作</p><h2>${escapeHtml(content.Title || '已返回内容')}</h2><p>${escapeHtml(content.Summary || '接口已成功返回结构化数据。')}</p>${content.Url ? `<a href="${escapeHtml(content.Url)}" rel="noreferrer">查看知乎原文</a>` : ''}</section>` : '';
...
export default async function handler(request, response) {
  ...
  try {
    ...
    const [profilePayload, contentPayload] = await Promise.all([
      fetchJson('https://openapi.zhihu.com/user', { headers: commonHeaders }).catch(() => null),
      fetchJson('https://developer.zhihu.com/api/v1/user/contents?ContentType=all&Limit=1&Offset=0&SortField=ts&SortOrder=desc', { headers: commonHeaders }).catch(() => null),
    ]);
    const profile = profilePayload?.data || profilePayload?.Data || profilePayload?.user || null;
    const content = contentPayload?.Data?.Items?.[0] || null;
    response.status(200).send(htmlPage({
      title: '知乎授权成功',
      message: '授权码已在服务端交换，下面是最小化的联调结果。',
      profile,
      content,
